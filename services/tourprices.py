...
class TourPricesService:
    ...
    async def _create_prices_for_tour(self, price_data: CreateTourPriceSchema):
        ...
        for target_currency in target_currencies:
            if target_currency == base_currency:
                ...
                exchange_rate = await CurrencyHandler.get_exchange_rate(
            ...
            price_data_dict = self._create_price_data_dict(
                price_data.tour_id, target_currency.id, converted_price, price_data
            )
            response.append(TourPriceSchema(**price_data_dict))
    ...
    async def update_tour_prices(
        self, tour_id: int, price_data: UpdateTourPriceSchema
        ...
        async with self.uow:
            ...
            for price in prices:
                ...
                price_data_dict = self._create_price_data_dict(
                    price_data.tour_id, price.currency_id, converted_price, price_data
                )
                updated_price = await self.uow.tour_prices.update(price.id, price_data_dict)
    ...
    async def _create_price_data_dict(
        self, tour_id, currency_id, price, price_data
    ):
        if price_data.new_price is not None and price_data.discount_percentage is not None:
            raise CustomExceptions.conflict(
                "You can only fill either the new price or the discount"
            )

        if price_data.discount_percentage:
            new_price = await self._calculate_new_price(price, price_data.discount_percentage)
