...
class CurrenciesService:
    ...
    async def create_currency(self, currency_data: CreateCurrencySchema) -> Currency:
        currency_dict = currency_data.model_dump()
        async with self.uow:
            return await self.uow.currencies.create(currency_dict)
        
    async def get_list_of_currencies(self, pagination: Pagination) -> list[Currency]:
        async with self.uow:
    ...
    async def update_currency(self, currency_id: int, currency_data: UpdateCurrencySchema) -> Currency:
        currency_dict = currency_data.model_dump()
        async with self.uow:
            return await self.uow.currencies.update(currency_id, currency_dict)
        
    async def delete_currency(self, currency_id: int) -> Currency:
        async with self.uow:
