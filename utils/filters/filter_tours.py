from fastapi import Query
from fuzzywuzzy import fuzz
import models
from utils.locale_handler import LocaleHandler
from utils.filters.base import BaseFilterParams


class FilterToursParams(BaseFilterParams):
    def __init__(
        self,
        country_id: int = Query(0),
        region_id: int = Query(0),
        start_month: int = Query(0),
        query: str = Query(0),
        country_name: str = Query(0),
        user_id: int = Query(0),
        rating: int | float = Query(0),
        status_id: int = Query(0),
        duration_from: int = Query(0),
        duration_to: int = Query(0),
        category_id: int = Query(0),
        type_id: int = Query(0),
        accommodation_id: int = Query(0),
        age_group_from: int = Query(0),
        age_group_to: int = Query(0),
        is_one_day_tour: bool = Query(False),
        children_age_id: int = Query(0),
        activity_level_id: int = Query(0),
        language_id: int = Query(0),

        is_price_for_day: bool = Query(False),
        currency_id: int = Query(0),

        price_from: float = Query(0),
        price_to: float = Query(0),

        only_with_discounts: bool = Query(False),

    ):
        self.country_id = country_id
        self.region_id = region_id
        self.start_month = start_month 
        self.query = query
        self.country_name = country_name
        self.user_id = user_id
        self.rating = rating
        self.status_id = status_id
        self.duration_from = duration_from
        self.duration_to = duration_to
        self.category_id = category_id
        self.type_id = type_id
        self.accommodation_id = accommodation_id
        self.age_group_from = age_group_from
        self.age_group_to = age_group_to
        self.is_one_day_tour = is_one_day_tour
        self.children_age_id = children_age_id
        self.activity_level_id = activity_level_id
        self.language_id = language_id

        self.is_price_for_day = is_price_for_day
        self.currency_id = currency_id
        self.price_from = price_from
        self.price_to = price_to

        self.only_with_discounts = only_with_discounts



    def filter_item(self, tour: models.Tour, locale: LocaleHandler):
        # Список условий фильтрации
        filters = []

        #проверка параметра country_id
        if self.country_id:
            filters.append(self.country_id in tour.country_ids)

        if self.region_id:
            filters.append(self.region_id in tour.region_ids)

        #проверка параметра start_month
        if self.start_month:
            filters.append(self.start_month == tour.start_month)

        # Проверка параметра query
        if self.query != "0":
            filters.append(
                fuzz.partial_ratio(
                    self.query.lower(), tour.title[locale.get_language].lower()
                ) >= 60
            )

        if self.country_name != "0":
            filters.append(self.country_name.lower() in [country.name[locale.get_language].lower() for country in tour.countries])

        if self.user_id:
            filters.append(tour.user_id == self.user_id)

        if self.rating:
            filters.append(self.rating == tour.user.rating)

        # Проверка параметра status_id
        if self.status_id:
            filters.append(tour.status_id == self.status_id)

        # Проверка параметра duration_from
        if self.duration_from:
            filters.append(tour.duration >= self.duration_from)

        # Проверка параметра duration_to
        if self.duration_to:
            filters.append(tour.duration <= self.duration_to)

        #Проверка параметра category_id
        # if self.category_id is not None:
        #     filters.append(self.category_id in tour.category_ids)
        if self.category_id:
            filters.append(self.category_id == tour.category_id)

        if self.type_id:
            filters.append(self.type_id == tour.main_type_id)

        if self.accommodation_id:
            filters.append(self.accommodation_id in tour.accommodation_ids)

        if self.age_group_from:
            filters.append(tour.age_group_from >= self.age_group_from)

        # Проверка параметра age_group_to
        if self.age_group_to:
            filters.append(tour.age_group_to <= self.age_group_to)
        # Проверка параметра is_one_day_tour
        if self.is_one_day_tour:
            filters.append(tour.is_one_day_tour == self.is_one_day_tour)

        # Проверка параметра children_age_id
        if self.children_age_id:
            filters.append(tour.children_age_id == self.children_age_id)

        # Проверка параметра activity_level_id
        if self.activity_level_id:
            filters.append(tour.activity_level_id == self.activity_level_id)

        if self.language_id:
            filters.append(self.language_id in tour.language_ids)

        if self.currency_id:
            price_filters = []
            
            for tour_price in tour.prices:
                if self.is_price_for_day:
                    price_filters.append(
                        self.currency_id == tour_price.currency_id and
                        self.price_from <= tour_price.price_for_day <= self.price_to
                    )
                else:
                    price_filters.append(
                        self.currency_id == tour_price.currency_id and
                        self.price_from <= tour_price.price <= self.price_to
                    )
            
            # Use any() to check if at least one price filter condition is satisfied
            filters.append(any(price_filters))

        if self.only_with_discounts:
            filters.append(self.only_with_discounts == tour.has_discount)
        # Применение всех условий с использованием логического оператора "и" (AND)
        return all(filters)
