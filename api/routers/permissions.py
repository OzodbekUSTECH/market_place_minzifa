...
from utils.permissions import read_permissions, read_permission, create_permission, update_permission, delete_permission
router = APIRouter(
    prefix="/permessions",
    tags=["Permissions"],
)
...
@router.get('/{permission_id}', name="get permission by ID", response_model=PermissionSchema, dependencies=[Depends(read_permission)])
